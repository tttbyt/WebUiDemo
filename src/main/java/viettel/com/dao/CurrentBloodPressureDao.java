...
@Repository
public class CurrentBloodPressureDao {

	@Autowired
	private CassandraOperations cassandraConnecttion;

	public BloodPressureCurrent getCurrent(String uUID) {
		return cassandraConnecttion.selectOne(QueryBuilder.select().from(WebConstant.BP_CURRENT_TABLE).where(QueryBuilder.eq("id", UUID.fromString(uUID))), BloodPressureCurrent.class);
	}
}
